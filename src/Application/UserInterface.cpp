...
{
	float xCentre = (((float)Window::instance()->width) - width) / 2;
	float yCentre = (((float)Window::instance()->height) - height) / 2;

	vec2s spriteOffset = { .x = 1, .y = 1 };
	vec2s buttonSize = { width, 128.f };
	vec2s subSpriteSize = { width, 128.f };

	renderer2D->addQuad({ xCentre, yCentre - 40.f, 0.5f }, buttonSize, subSpriteSize, { 0, 3 }, spriteOffset);
	addButton({ xCentre, yCentre + 100.f, 0.5f }, buttonSize, subSpriteSize, { 0, 2 }, spriteOffset, START_BUTTON);
	addButton({ xCentre, yCentre + 240.f, 0.5f }, buttonSize, subSpriteSize, { 0, 1 }, spriteOffset, EXIT_BUTTON);
}
...
void GUI::addButton(vec3s position, vec2s size, vec2s spriteSize, vec2s rowAndColumn, vec2s offset, UI uiElement)
{
	renderer2D->addQuad(position, size, spriteSize, rowAndColumn, offset);
