...
static void createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags, VkImage& image, VkDeviceMemory& imageMemory)
{
...
static VkImageView createTextureImageView(const VkImage& image, VkFormat format) 
{
    ...
    if (vkCreateImageView(device, &createImageViewInfo, nullptr, &textureImageView) != VK_SUCCESS)
    {
...
static void createImageViews() 
{
    ...
    for (uint32_t i = 0; i < swapchainImages.size(); ++i)
    {
        ...
        createImageViewInfo.viewType = VK_IMAGE_VIEW_TYPE_2D;
        createImageViewInfo.format = swapchainFormat;
        createImageViewInfo.components.r = VK_COMPONENT_SWIZZLE_IDENTITY;
        createImageViewInfo.components.g = VK_COMPONENT_SWIZZLE_IDENTITY;
        createImageViewInfo.components.b = VK_COMPONENT_SWIZZLE_IDENTITY;
        createImageViewInfo.components.a = VK_COMPONENT_SWIZZLE_IDENTITY;
...
int main()
{
    ...
    char* binFragCode = fileReadBinary("Shaders/mainShader.frag.spv", &sizeOfFragmentCode);
    ...
    VkCommandPoolCreateInfo poolTransferCreateInfo{};
    poolTransferCreateInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
    poolTransferCreateInfo.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
    poolTransferCreateInfo.queueFamilyIndex = transferQueueFamilyIndex;

    if (vkCreateCommandPool(device, &poolTransferCreateInfo, nullptr, &transferCommandPool))
    {
        assert(false);
        printf("Failed to create a command pool.");
    }
    ...
    while (running)
    {
        ...
        VkClearValue clearColour = { { { 0.f, 0.535f, 1.f, 1.f } } };
